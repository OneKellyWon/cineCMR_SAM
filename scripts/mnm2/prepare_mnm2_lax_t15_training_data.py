...
OUT_DATA_DIR = PROJECT_ROOT / "example_data/data/MnM2_LAX_T15"
OUT_PATIENT_LIST = PROJECT_ROOT / "example_data/data/Patient_list/patient_list_mnm2_lax_t15.xlsx"
OUT_QA_FILE = PROJECT_ROOT / "outputs/mnm2_lax_t15_preprocess_check.xlsx"
...
def select_frames(t, ed_frame, es_frame, target_t):
    ...
    if len(selected) != target_t:
        raise RuntimeError(f"Expected {target_t} selected frames, got {len(selected)}: {selected}")
...
def main():
    ...
    for _, row in summary.iterrows():
        patient_id = Path(row["cine_file"]).parent.name

        if row["status"] != "ok":
            skipped_rows.append({
                "patient_id": patient_id,
                "reason": f"summary_status_{row['status']}",
            })
            continue
        ...
        if selected is None:
            skipped_rows.append({
                "patient_id": patient_id,
                "reason": f"original_time_frames_lt_{TARGET_T}",
                "original_time_frames": t,
            })
            continue

        out_cine = cine[:, :, selected]
        if out_cine.shape[-1] != TARGET_T:
            raise RuntimeError(f"Unexpected output cine shape for {patient_id}: {out_cine.shape}")
    ...
    skipped_file = OUT_QA_FILE.with_name("mnm2_lax_t15_skipped_cases.xlsx")
    skipped_df.to_excel(skipped_file, index=False)

    print(f"Saved patient list: {OUT_PATIENT_LIST}")
    print(f"Saved QA file: {OUT_QA_FILE}")
    print(f"Saved skipped cases: {skipped_file}")
    print(f"Kept cases: {len(patient_df)}")
    print(f"Skipped cases: {len(skipped_df)}")
    if len(qa_df) > 0:
        print("Original time frame summary for kept cases:")
