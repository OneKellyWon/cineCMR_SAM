# customized image encoder for cineCMR-SAM
...
class Block_timesformer(nn.Module):
    "Paper : TimeSformer"
    "https://github.com/facebookresearch/TimeSformer/blob/main/timesformer/models/vit.py#L178"
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_rel_pos:
            print('in attention we use rel pos - add by zhennong in custom_ImageEncoder.py')
...
class ImageEncoderViT_medivista(nn.Module):
    def __init__(
        self,
        args,
        chunk,
        ...
        # print('in custom_ImageEncoder.py, img_size: ', img_size)
        ...
        self.pe = PositionalEncodingPermute2D(embed_dim)

        # designed for image size = 128
        self.inc = DoubleConv(self.in_chans, 8)
        self.down1 = Down(8, 16)
        self.down2 = Down(16, 32)
        self.down3 = Down(32, embed_dim)
    


        self.out_conv = nn.Sequential(
            nn.Conv2d(embed_dim, out_chans, kernel_size=3, padding=1),
